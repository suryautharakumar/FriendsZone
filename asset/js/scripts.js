...
-----------------------------------------------------------------------------------*/
let currentIndex = 0;
const images = document.querySelectorAll('.logoImg');

// Open modal function and display the current image
function openModal(index) {
    ...
    // Add keydown event listener for left and right arrow keys
...
// Function to change the image in fullscreen view
...
function handleKeyNavigation(event) {
    if (event.key === "ArrowRight") {
        changeImage(1); // Right arrow key moves to the next image
    } else if (event.key === "ArrowLeft") {
        changeImage(-1); // Left arrow key moves to the previous image
    }
}
...
images.forEach((image, index) => {
    image.addEventListener('click', () => {
        openModal(index); // Open modal with the clicked image
    });
});
